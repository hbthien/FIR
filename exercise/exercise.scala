...
val tokenizer2 = new Tokenizer().setInputCol("text").setOutputCol("words")

val wordsTesting = tokenizer2.transform(test)

//converts the words column into feature vectors and add to data frame
val hashingTF2 = new HashingTF().setInputCol("words").setOutputCol("rawFeatures").setNumFeatures(20)
val featurizedTestData = hashingTF2.transform(wordsTesting)

 
val idf2 = new IDF().setInputCol(hashingTF.getOutputCol).setOutputCol("features")
val idfTestModel = idf2.fit(featurizedTestData)
